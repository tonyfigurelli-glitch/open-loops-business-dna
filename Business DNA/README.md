...
AI-assisted participant narrative uses the traceable instruction version `small_business_owner_v1.4_ai_generation@2.1.0`. It preserves the ten canonical Version 1.3 section IDs, titles, and order while requiring section bodies to read as polished reflection rather than expose calibration machinery, evidence labels, metadata, prompt text, raw answer summaries, confidence ratings, internal evaluation rationale, answer quotations, or lightly reworded answers. Exact answers remain available only in the collapsed inspectable record. The deterministic fallback now derives cross-answer operating signals and must state when evidence is insufficient rather than fill sections with answer text.
