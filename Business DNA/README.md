...
AI-assisted participant narrative uses the traceable instruction version `small_business_owner_v1.3_ai_generation@1.1.1`. It preserves the ten canonical Version 1.3 section IDs and titles while requiring section bodies to read as polished reflection rather than expose calibration machinery, evidence labels, metadata, prompt text, or raw answer summaries. Frozen labeled structures are treated as semantic requirements rendered into natural prose; full experiment details remain in the structured experiment field. Narrative validation remains part of the same retry, evidence, safety, provenance, immutability, and deterministic-fallback boundary.
